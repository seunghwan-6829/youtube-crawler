    ...
    <div className='min-h-screen' style={{background: '#fafafa'}}>
      <nav className='navbar fixed top-0 left-0 right-0 z-50'>
        <div className='max-w-6xl mx-auto px-6 py-4 flex justify-between items-center'>
          <Link href='/dashboard' className='flex items-center gap-2'>
            ...
            <span className='text-xl font-bold'><span className='gradient-text'>Reboot</span> Crawler</span>
          </Link>
          <div className='flex items-center gap-6'>
            {isAdmin && <Link href='/admin' className='text-orange-500 hover:text-orange-600 font-medium'>Admin</Link>}
      ...
      <main className='pt-24 pb-16'>
        <div className='max-w-4xl mx-auto px-6'>
          <div className='mb-8'>
            <h1 className='text-3xl font-bold mb-2'>YouTube Search</h1>
            <p className='text-gray-500'>Enter keywords to search YouTube videos</p>
          </div>

          <form onSubmit={handleSearch} className='flex gap-4 mb-8'>
            <input type='text' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              className='input-field flex-1' placeholder='Enter search query...' />
            ...
            {videos.map((video) => (
              <div key={video.id} className='card flex gap-4 hover:shadow-lg transition-shadow'>
                ...
                <a href={'https://youtube.com/watch?v=' + video.id} target='_blank' rel='noopener noreferrer'
                  className='btn-primary text-sm py-2 px-4 self-center flex-shrink-0'>Watch</a>
              </div>
            ))}
          </div>
          ...
          {videos.length === 0 && !loading && !error && (
            <div className='text-center py-20'>
              ...
              <p className='text-gray-500'>Enter a search query to find YouTube videos</p>
            </div>
          )}
        </div>
      </main>
